...
void timerHandle(struct StackFrame *sf){
	...
	//TODO check
		...
		if(pcb[current].state != STATE_RUNNING || ++pcb[current].timeCount >= MAX_TIME_COUNT) {
				...
				int i = 1;
				for(; i < MAX_PCB_NUM ;i++) {
						int pro_i = (current + i) % MAX_PCB_NUM;
						if(pcb[pro_i].state == STATE_RUNNABLE) {
								next = pro_i;
								break;
						}
				}

				if(i == MAX_PCB_NUM) next = 0;
				...
				/*if(next != current) {*/
						...
						/*checkyy ++;*/
						/*checkyy %=  50;*/
						/*putChar('0'+checkyy);putChar('0'+ current);putChar('\n');*/
...
void syscallFork(struct StackFrame *sf){
	...
	//TODO:checked 查找空闲pcb，如果没有就返回-1
	int i=0;
	for(;i < MAX_PCB_NUM; i++) {
...
/*for (int j = 0; j < 0x100000; j++) {*/
		...
		/*if(!(j%0x100))asm volatile("int $0x20"); //XXX Testing irqTimer during syscall*/
/*}*/
/*disableInterrupt();*/
...
void syscallExit(struct StackFrame *sf){
	//TODO checked 先设置成dead，然后用int 0x20进入调度
